"use client" // Pindahkan logic ke client component jika ingin animasi, atau tetap server component untuk SEO
...
/* ===== UPGRADED KPI COMPONENT ===== */

function KpiCard({ 
  title, value, icon: Icon, color, href, subtitle 
}: { 
  title: string, value: number | string, icon: any, color: string, href?: string, subtitle?: string 
}) {
  return (
    <motion.div 
      whileHover={{ y: -4 }}
    ...
    >
      ...
      {/* Progress bar tipis buat estetika */}
      <div className="mt-4 h-1 w-full bg-gray-50 rounded-full overflow-hidden">
        <div className={`h-full ${color.mainBar} w-2/3 opacity-40`} />
      </div>
    </motion.div>
  )
}
...
export default function AdminDashboardPage({ data }: any) {
  // Anggap data diambil dari parent server component
    ...
    <section className="p-8 space-y-12 bg-[#f8fafc] min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Monitoring operasional PT MPP secara realtime.</p>
        </div>
      ...
      {/* GRID STRATEGY: Groups by Logic */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        
        {/* FINANCE & PROJECT (URGENT) */}
        <KpiCard 
      ...
      {/* SECONDARY ROW - Tables or Charts could go here */}
      <div className="grid lg:grid-cols-3 gap-8">
         {/* Misal lo mau tambahin tabel mini buat GA yang dokumennya expired */}
         <div className="lg:col-span-2 bg-white border rounded-2xl p-6 shadow-sm">
            <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
               <FileText size={18} className="text-blue-600"/> 
               Peringatan Dokumen & Kontrak
            </h3>
            {/* List mini dokumen expired */}
            <div className="space-y-3">
               {[1,2,3].map(i => (
                 <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100">
                    <div className="flex items-center gap-3">
                       <div className="w-2 h-2 bg-red-500 rounded-full"/>
                       <p className="text-sm font-medium text-gray-700 font-mono text-[11px]">DOC-EXP-00{i}</p>
                    </div>
                    <span className="text-[10px] font-bold text-red-500 bg-red-50 px-2 py-1 rounded">EXPIRED</span>
                 </div>
               ))}
            </div>
         </div>
         ...
         <div className="bg-blue-600 rounded-2xl p-6 text-white shadow-xl shadow-blue-200">
            <h3 className="font-bold opacity-80 text-sm uppercase">Total Aset Perusahaan</h3>
