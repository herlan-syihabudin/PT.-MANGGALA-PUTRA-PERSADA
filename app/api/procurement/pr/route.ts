...
// ========== PROCUREMENT SHEETS (different ID) ==========
...
async function createPRFromApprovedMR(requestNo: string) {
  try {
    if (!PROCUREMENT_SHEET_ID) {
      console.error("GSHEET_PROCUREMENT_ID is not defined")
      return false
    }
...
// ambil request_no dari row yg dipilih
...
for (const idx of allIndexes) {
  await sheets.spreadsheets.values.update({
    ...
    // ===== 🔥 CRITICAL: If Approved, create PR with ALL items in this request_no =====
    if (status === "Approved") {
      console.log(`MR Approved: ${requestNo}. Creating PR with all items...`)
      
      // Call the function to create PR with ALL items in this request
      const prCreated = await createPRFromApprovedMR(requestNo)
      
      if (!prCreated) {
        console.warn(`PR creation failed for MR: ${requestNo}, but status updated to Approved`)
        // We don't rollback the status, just log the warning
      }
    }
