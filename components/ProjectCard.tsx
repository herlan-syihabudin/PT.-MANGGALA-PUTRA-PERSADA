...
export default function ProjectCard({
  ...
  return (
    ...
    >
      <article className="relative border border-gray-200 rounded-2xl overflow-hidden bg-white hover:shadow-2xl transition-all duration-300 h-full flex flex-col hover:-translate-y-1">
        ...
        {featured && (
          <div className="absolute top-4 left-4 z-20 flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-yellow-400 to-yellow-300 text-gray-900 rounded-full text-xs font-bold shadow-lg">
            <Award size={12} />
        ...
        <div className="relative h-56 sm:h-64 overflow-hidden bg-gray-100">
          ...
          <div className="absolute bottom-4 left-4">
            <span className="px-3 py-1.5 bg-red-600 text-white text-xs font-semibold rounded-full">
              {category}
            </span>
          </div>

          {images?.length > 1 && (
            <div className="absolute top-4 right-4 px-2 py-1 bg-black/50 text-white text-xs rounded-full">
        ...
        <div className="p-6 flex-1 flex flex-col">
          ...
          <div className="mt-4 flex items-center text-sm font-semibold text-red-600">
  ...
  const hasStats = client || projectValue
  ...
  return (
    ...
    >
      <article className="relative border border-gray-200 rounded-2xl overflow-hidden bg-white hover:shadow-2xl transition-all duration-300 h-full flex flex-col hover:-translate-y-1">
        ...
        <div className="relative h-56 sm:h-64 overflow-hidden bg-gray-100">
          {/* Image dengan overlay gradient */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent z-10 opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden="true" />
          
          <Image
            src={hasImages ? images[0] : "/images/project-placeholder.jpg"}
            alt={title}
            fill
            sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
            className="object-cover group-hover:scale-110 transition duration-700"
          ...
          {/* Image count indicator (if multiple images) */}
          {images.length > 1 && (
            <div className="absolute top-4 right-4 z-20 px-2 py-1 bg-black/50 backdrop-blur-sm text-white text-xs rounded-full">
              +{images.length} foto
            </div>
          )}
        </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-2 gap-4">
              {client && (
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900 truncate" title={client}>
                    {client}
                  </p>
                </div>
              )}
              {projectValue && (
                <div>
                  <p className="text-xs text-gray-400">Nilai Proyek</p>
                  <p className="text-sm font-medium text-green-600">{projectValue}</p>
                </div>
              )}
            </div>
          )}
        ...
        {/* ===== HOVER EFFECT OVERLAY ===== */}
        <div className="absolute inset-0 border-2 border-transparent group-hover:border-red-600/20 rounded-2xl pointer-events-none transition-colors" aria-hidden="true" />
      </article>
    </Link>
  )
}
